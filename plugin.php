<?php
/**
 * Plugin Name:       The Events Calendar Extension: Remove Past Ignored Events
 * Plugin URI:        
 * GitHub Plugin URI: https://github.com/mt-support/tec-labs-remove-past-ignored-events
 * Description:       
 * Version:           1.0.0
 ...
 * @since 1.0.0
 */
function tribe_extension_remove_past_ignored_events() {
	...
	Tribe__Autoloader::instance()->register_prefix(
		'\\Tribe\\Extensions\\Remove_Past_Ignored_Events\\',
		__DIR__ . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'Tec',
		'remove-past-ignored-events'
	);

	// Deactivates the plugin in case of the main class didn't autoload.
	if ( ! class_exists( '\Tribe\Extensions\Remove_Past_Ignored_Events\Plugin' ) ) {
		tribe_transient_notice(
			'remove-past-ignored-events',
			'<p>' . esc_html__( 'Couldn\'t properly load "The Events Calendar Extension: Remove Past Ignored Events" the extension was deactivated.', '__TRIBE_DOMAIN__' ) . '</p>',
			[],
	...
	tribe_register_provider( '\Tribe\Extensions\Remove_Past_Ignored_Events\Plugin' );
}

// Loads after common is already properly loaded.
add_action( 'tribe_common_loaded', 'tribe_extension_remove_past_ignored_events' );
