...
* `Remove Past Ignored Events` - Plugin Human-readable name, e.g. "Sample Extension"
* `Remove_Past_Ignored_Events` - Which namespace we will use for the plugin. Usually the plugin name or the slug in uppercase without spaces, e.g. "SampleExtension"
* `remove-past-ignored-events` - Uses using dashes normally, e.g. "sample-extension"
* `__TRIBE_DOMAIN__` - Translation domain normally with "tribe-" prefix, e.g. "tec-labs-sample-extension"
* `remove_past_ignored_events` - Uses underscores, so it's safe for variables, e.g. "sample_extension"
